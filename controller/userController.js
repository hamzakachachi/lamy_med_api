...
const getMyInfos= async (req,res)=>{
    try {
        if (req.params.username === req.decoded.username) {
            const resultats=await userModel.findOne({username:req.params.username},{password:0});
            res.status(200).json(resultats);
        } else {
...
module.exports={
    getAll, getOneByMat, getUsersBySecteur, getMyInfos, getOneByUserName, updateMyInfos, addUser, updateUser, deleteUser
}
